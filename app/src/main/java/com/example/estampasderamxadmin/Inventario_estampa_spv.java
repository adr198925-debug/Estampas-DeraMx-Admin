...
public class Inventario_estampa_spv extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.inventario_estampa_spv);

        recyclerView = findViewById(R.id.recyclerInventario);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        // Limpieza de seguridad al iniciar la pantalla para evitar duplicados
        listaTotal.clear();
        disponibles.clear();
        ...
        refE = FirebaseDatabase.getInstance().getReference("Spiderverse/estampas");
        refEs = FirebaseDatabase.getInstance().getReference("Spiderverse/especiales");
        refT = FirebaseDatabase.getInstance().getReference("Spiderverse/tarjetas");
    ...
    private void guardarLocalmente() {
        SharedPreferences pref = getSharedPreferences("Stock_Spiderverse", MODE_PRIVATE);
        ...
        if (datos != null) {
            ...
            // commit() síncrono para asegurar la persistencia física en el almacenamiento
            editor.commit();
        }
    }
    ...
    private void cargarNodo(DatabaseReference ref, String prefijo) {
        ref.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                // Sincronización con el almacenamiento local después de cada nodo
                cargarLocalmente();
